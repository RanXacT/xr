// ######################################################################################### - FILE
/*!
    This file contains external code adapted to fit into XR interface.

    External code source:
    https://database.cs.brown.edu/svn/hstore/trunk/src/dtxn/logging/crc32ctables.cc
...
uint32_t DoCRC32C_Hardware_32(uint32_t crc, const void* data, size_t length);
uint32_t DoCRC32C_Hardware_64(uint32_t crc, const void* data, size_t length);
uint32_t DoCRC32C_Software(uint32_t crc, const void* data, size_t length);

typedef uint32_t (*CRC32CFunctionPtr)(uint32_t crc, const void* data, size_t length);
#if XR_COMPILER_MICROSOFT
#define XR_CRC32_U8   _mm_crc32_u8
#define XR_CRC32_U16  _mm_crc32_u16
#define XR_CRC32_U32  _mm_crc32_u32
#define XR_CRC32_U64  _mm_crc32_u64
#define XR_CPUID    __cpuid

#else

#define XR_CRC32_U8   __builtin_ia32_crc32qi
#define XR_CRC32_U16  __builtin_ia32_crc32hi
#define XR_CRC32_U32  __builtin_ia32_crc32si
#define XR_CRC32_U64  __builtin_ia32_crc32di


//  GCC Inline Assembly to emulate VS intrinsic
void XR_CPUID(int CPUInfo[4],int InfoType){
...
#if defined(XR_CPU_X64) || defined(XR_CPU_X86)
    static const int SSE42_BIT = 20;
    int regs[4];
    XR_CPUID(regs, 1);
    if(regs[2] & (1 << SSE42_BIT))
    {
#ifdef XR_CPU_X64
        return DoCRC32C_Hardware_64;
#else
        return DoCRC32C_Hardware_32;
#endif
    }
#endif

    // Fallback to software version.
    return DoCRC32C_Software;
}

static CRC32CFunctionPtr gCrc32Function = detectBestCRC32C();
...
uint32_t DoCRC32C_Hardware_64(uint32_t crc, const void* data, size_t length) {
    ...
    for (size_t i = 0; i < length / sizeof(uint64_t); i++) {
        crc64bit = XR_CRC32_U64(crc64bit, *(uint64_t*) p_buf);
    ...
    switch (length) {
        case 7:
            crc32bit = XR_CRC32_U8(crc32bit, *p_buf++);
        case 6:
            crc32bit = XR_CRC32_U16(crc32bit, *(uint16_t*) p_buf);
            p_buf += 2;
        // case 5 is below: 4 + 1
        case 4:
            crc32bit = XR_CRC32_U32(crc32bit, *(uint32_t*) p_buf);
            break;
        case 3:
            crc32bit = XR_CRC32_U8(crc32bit, *p_buf++);
        case 2:
...
uint32_t DoCRC32C_Hardware_32(uint32_t crc, const void* data, size_t length) {
    ...
    for (size_t i = 0; i < length / sizeof(uint32_t); i++) {
        crc = XR_CRC32_U32(crc, *(uint32_t*) p_buf);
    ...
    */
    switch (length) {
        case 3:
            crc = XR_CRC32_U8(crc, *p_buf++);
        case 2:
            crc = XR_CRC32_U16(crc, *(uint16_t*) p_buf);
            break;
        case 1:
            crc = XR_CRC32_U8(crc, *p_buf);
            break;
...
// --------------------------------------------------------------------------------------  FUNCTION
// --------------------------------------------------------------------------------------  FUNCTION
void CRC32C::Append(XR_IN_BYTES(sizeInBytes)const void *buffer, size_t sizeInBytes)
{
    XR_ASSERT_ALWAYS_FALSE(mFinalized);
    mState = gCrc32Function(mState, buffer, sizeInBytes);
}
// --------------------------------------------------------------------------------------  FUNCTION
// --------------------------------------------------------------------------------------  FUNCTION
XR_NO_SIDE_EFFECTS_PREFIX
    uint32_t CRC32C::Process(XR_IN_STRING const char * s)
{
    return gCrc32Function(0, s, xr::Core::StringLengthWithNull(s)-1 );
}
// --------------------------------------------------------------------------------------  FUNCTION
// --------------------------------------------------------------------------------------  FUNCTION
XR_NO_SIDE_EFFECTS_PREFIX
    uint32_t CRC32C::ProcessAppend(uint32_t previousCRC, const char * s)
{
    return gCrc32Function(previousCRC, s, xr::Core::StringLengthWithNull(s)-1 );
}
// --------------------------------------------------------------------------------------  FUNCTION
// --------------------------------------------------------------------------------------  FUNCTION
...
// --------------------------------------------------------------------------------------  FUNCTION
// --------------------------------------------------------------------------------------  FUNCTION
void CRC32C::GetResult(Digest *d)
{
